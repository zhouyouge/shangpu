...
/*        alert("param:"+JSON.stringify(param));*/
    ...
    //品牌信息全查
    if(yemian=="Brandinformation"){
        if (param == null) {
            param = {};
            param.curPage = 1;
            param.PageSize = 5;
        }
        var brandCoding=$("#brandCoding").val();
        var startTime=$("#datemin").val();
        var outTime=$("#datemax").val();
        if(startTime!=""){
            param.startTime=new Date(startTime);
        ...
        if(brandCoding!=null){
            param.brandCoding=brandCoding;
        }
        alert("品牌信息全查");
        $.ajax({
            url: "http://localhost:8080/ShopManagement/queryBrandinformation",
            type: "post",
            data: param,
            dataType: "html",
            success: function (result) {
