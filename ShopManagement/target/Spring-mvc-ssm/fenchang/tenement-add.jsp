...
<script>
	$(function () {
		$("#id").val("");
		$("#type").val("");
		if(${sessionScope.tenementId!=null}){
			var tenementId=<%=session.getAttribute("tenementId")%>;
			$("#type").val(2);
			$("#id").val(tenementId);
			$("#biaoti").text("修改租户信息");
			$("#xiugaitenementsubmit").show();
			$("#addtenementsubmit").hide();
			$("#zhengzhao").hide();
			xiugaiTement(tenementId);
			<%session.removeAttribute("tenementId");%>;
		}else{
			$("#xiugaitenementsubmit").hide();
			$("#addtenementsubmit").show();
		}
	});
</script>
<!--_meta 作为公共模版分离出去-->
...
<head>
	...
	<title class="title1">新增租户信息</title>
...
<article class="page-container">
	<%--<form class="form form-horizontal" &lt;%&ndash;id="form-article-add"&ndash;%&gt;>--%>
		...
		<br/>
		<div id="biaoti" class="title1" >新增租户信息</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
			<select id="tenantType" class="select">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select id="managementForms" class="select">
					<option id="managementFormsMoren">业态，必填</option>
				</select>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select id="tenanStatus" class="select">
					<option id="tenanStatusMoren">租户状态，必填</option>
					<option id="qiyong" >启用</option>
					<option id="tingzhi">停用</option>
				</select>
			</div>
		</div>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="xiugaitenementsubmit();" class="btn btn-primary radius" id="xiugaitenementsubmit" ><i class="Hui-iconfont">&#xe632;</i> 修改并提交审核</button>
				<button onClick="addtenementsubmit();" class="btn btn-primary radius"  id="addtenementsubmit"><i class="Hui-iconfont">&#xe632;</i> 保存并提交审核</button>
				<%--<button onClick="article_save();" class="btn btn-secondary radius" type="button"><i class="Hui-iconfont">&#xe632;</i> 保存草稿</button>--%>
				<button onClick="removeTenment();" class="btn btn-default radius" type="button">&nbsp;&nbsp;取消&nbsp;&nbsp;</button>
			</div>
		</div>
