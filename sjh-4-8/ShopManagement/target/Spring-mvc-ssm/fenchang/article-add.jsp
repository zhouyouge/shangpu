...
<script>
    $(function () {
        ...
        var breakId='<%=session.getAttribute("breakId")%>';
        if(${sessionScope.breakId!=null}){
            alert(breakId);
            $("#id").val(breakId);
            $("#Addjiafang").show();
            $("#AddKaipiao").hide();
            ...
            $("#addarticle1").hide();
            $("#title").text("修改甲方合同信息");
            $("#breakId").attr('readOnly','readOnly');
            updateTownercontract(breakId);
            <%session.removeAttribute("breakId");%>
        }
        var bankId='<%=session.getAttribute("bankId")%>';
        if(${sessionScope.bankId!=null}){
            alert(bankId);
            $("#id").val(bankId);
            $("#Addjiafang").hide();
            $("#AddKaipiao").hide();
            ...
            updateBankAccount(bankId);
            <%session.removeAttribute("bankId");%>
        }
        var billId='<%=session.getAttribute("billId")%>';
        if(${sessionScope.billId!=null}){
            alert(billId);
            $("#id").val(billId);
            $("#Addjiafang").hide();
            $("#AddKaipiao").show();
...
<article class="page-container"><br/>
	...
	<div class="title1" id="title">新增分场信息</div>
      ...
      <div id="Addjiafang">
		...
		<input type="hidden" id="breakoutNamee">
		<div class="row cl">
      ...
      <div id="AddKaipiao">
		...
		<div class="title1">新增开票信息</div>
		  ...
		  <input type="text" id="breakoutNameee">
		<div class="row cl">
