...
public class PaymenttermServiceImpl implements PaymenttermService {
    ...
    @Override
    public ResponseResult toupdateChaPaymentterm(Paymentterm paymentterm) {
        ResponseResult rs = new ResponseResult();
        if (EmptyUtils.isEmpty(paymentterm)) {
            rs.setFlag(1);
            rs.setResult(false);
            ...
            Paymentterm house2 = paymenttermDao.toUpdatePaymenttermWhereOut(paymentterm);
    ...
    @Override
    public ResponseResult updatePaymentterm(Paymentterm paymentterm) {
        ResponseResult rs = new ResponseResult();
        if (EmptyUtils.isEmpty(paymentterm)) {
            rs.setFlag(1);
            rs.setResult(false);
            ...
            boolean flag = paymenttermDao.updatePaymentterm(paymentterm) > 0;
            if (flag) {
                rs.setFlag(2);
                rs.setResult(true);
                rs.setData("修改成功收款方");
    ...
    @Override
    public ResponseResult deletepaymentterm(Paymentterm towner1) {
        ResponseResult rs = new ResponseResult();
        if (EmptyUtils.isEmpty(towner1)) {
            rs.setFlag(1);
            rs.setResult(false);
            rs.setData("传参为空");
        } else {
            boolean flag = paymenttermDao.deletepaymentterm(towner1) > 0;
            if (flag) {
                rs.setFlag(2);
                rs.setResult(true);
                rs.setData("删除成功");
